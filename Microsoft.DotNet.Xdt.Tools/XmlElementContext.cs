...
namespace Microsoft.DotNet.Xdt.Tools
{
    internal class XmlElementContext : XmlNodeContext
    {
        ...
        private string ParentXPath => _parentXPath ?? (_parentXPath = ConstructParentXPath());
        ...
        private Locator CreateLocator(out string argumentString) {
            Locator locator = CreateObjectFromAttribute<Locator>(out argumentString, out _locatorAttribute);
        ...
        private XmlNodeList GetTargetNodes(string xpath) 
            => TargetDocument.SelectNodes(xpath, GetNamespaceManager());

        private Exception WrapException(Exception ex) 
            => XmlNodeException.Wrap(ex, Element);
        ...
        private XmlNameTable GetParentNameTable() => _parentContext == null 
            ? Element.OwnerDocument.NameTable 
            : _parentContext.GetNamespaceManager().NameTable;

        private static Regex _nameAndArgumentsRegex;
        private Regex NameAndArgumentsRegex 
            => _nameAndArgumentsRegex 
            ?? (_nameAndArgumentsRegex = new Regex(@"\A\s*(?<name>\w+)(\s*\((?<arguments>.*)\))?\s*\Z", RegexOptions.Compiled | RegexOptions.Singleline));
        ...
        internal bool HasTargetNode(out XmlElementContext failedContext, out bool existedInOriginal) {
            ...
            if (TargetNodes.Count != 0) return true;
        ...
        internal bool HasTargetParent(out XmlElementContext failedContext, out bool existedInOriginal) {
            ...
            if (TargetParents.Count != 0) return true;
