...
namespace Microsoft.DotNet.Xdt.Tools
{
    ...
    internal class XmlFormatter
    {
        ...
        public string CurrentAttributeIndent 
            => _currentAttributeIndent ?? (_currentAttributeIndent = ComputeCurrentAttributeIndent());
        ...
        private void ReorderNewItemsAtEnd(XmlNode node) {
            ...
            if (IsNewNode(node)) return;
            // If the last child isn't whitespace, new elements might
            // have been added
            XmlNode iter = node.LastChild;
            if (iter == null || iter.NodeType == XmlNodeType.Whitespace) return;
        ...
        private void SetIndent(string indent) {
            if (_currentIndent != null && _currentIndent.Equals(indent)) return;
        ...
        private bool NeedsIndent(XmlNode node, XmlNode previousNode) => !IsWhiteSpace(previousNode)
            && !IsText(previousNode)
            && (IsNewNode(node) || IsNewNode(previousNode));

        private static bool IsWhiteSpace(XmlNode node) => node != null
            && node.NodeType == XmlNodeType.Whitespace;

        private static bool IsText(XmlNode node) => node != null
            && node.NodeType == XmlNodeType.Text;

        private bool IsNewNode(XmlNode node) => node != null
        ...
        private string ComputeCurrentIndent() 
            => LookAheadForIndent() ?? PreviousIndent + OneTab;
        ...
        private static string ConvertIndentToTab(string indent) {
        ...
        private string ComputeCurrentAttributeIndent() 
            => LookForSiblingIndent(CurrentNode) ?? CurrentIndent + OneTab;
