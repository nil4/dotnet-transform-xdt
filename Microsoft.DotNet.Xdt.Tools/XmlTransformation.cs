...
{
    public sealed class XmlTransformation : IServiceProvider, IDisposable
    {
        internal const string TransformNamespace = "http://schemas.microsoft.com/XML-Document-Transform";
        private const string SupressWarnings = "SupressWarnings";
        ...
        private XmlTransformation(string transform, bool isTransformAFile = true, IXmlTransformationLogger logger = null) {
            _transformFile = transform;
            _logger = new XmlTransformationLogger(logger);

            _xmlTransformation = new XmlFileInfoDocument();// {PreserveWhitespace = true};
            if (isTransformAFile)
            {
                using (var stream = File.OpenRead(transform))
                {
        ...
        private bool HasTransformNamespace { get; set; }
        ...
        private void PreprocessTransformDocument() {
            ...
            if (!HasTransformNamespace) return;
